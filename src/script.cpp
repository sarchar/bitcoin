            ...
            switch (opcode)
            {
                ...
                case OP_HASH256:
                {
                    ...
                    if (opcode == OP_RIPEMD160)
                        RIPEMD160(vch.size() > 0 ? &vch[0] : (unsigned char*)"", vch.size(), &vchHash[0]);
                    else if (opcode == OP_SHA1)
                        SHA1(vch.size() > 0 ? &vch[0] : (unsigned char*)"", vch.size(), &vchHash[0]);
                    else if (opcode == OP_SHA256)
                        SHA256(vch.size() > 0 ? &vch[0] : (unsigned char*)"", vch.size(), &vchHash[0]);
